...
void SubmeshData::DrawSubmeshInstanced(unsigned int count)
{
	glDrawElementsInstancedBaseVertex(GL_TRIANGLES, mNumIndices, GL_UNSIGNED_INT, (void*)(sizeof(unsigned int)*mBaseIndex), count, mBaseVertex);
}
...
void MeshData::DrawInstanced(unsigned int count)
{
	for (int m = 0; m < mSubmesh.size(); m++)
	{
		mSubmesh[m].DrawSubmeshInstanced(count);
	}
}
